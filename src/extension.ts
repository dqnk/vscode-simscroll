...
export function activate(
    context: vscode
        .ExtensionContext) { //	for(let i = 0; i < editors.length; i++) {
  ...
  context.subscriptions.push(vscode.window.onDidChangeTextEditorVisibleRanges(
      ({textEditor, visibleRanges}) => {
        ...
        if (textEditor.document.fileName !== currFile) {
          return;
        }

        let critEditors = [];

        // currEditor would have to be recalculated in many cases such as other
        // windows being closed therefore it is just recalculated every time
        let currEditor = vscode.window.activeTextEditor;
        let allEditors = vscode.window.visibleTextEditors;
        let editors = [];
        for (let i = 0; i < allEditors.length; i++) {
          if (allEditors[i].document.fileName ===
        ...
        for (let i = 0; i < critEditors.length; i++) {
          ...
          critEditors[i].revealRange(
              new vscode.Range(new vscode.Position(anchor, 0),
                               new vscode.Position(anchor + height, 0)));
          anchor += height;
        }
      }));
  ...
  let enable =
      vscode.commands.registerCommand('simscroll.enableSimScroll', () => {
        toggle = 1;
        currFile = vscode.window.activeTextEditor?.document.fileName;
      });
  let disable = vscode.commands.registerCommand('simscroll.disableSimScroll',
                                                () => { toggle = 0; });
}
